...
Approaching software design can be overwhelming at first because you have an idea that you must break down into a list of features and systems, and then further break down those lists into smaller bite sized components. Keeping software agile and decoupled can be quite a challenge as it's easy to tie code together to get something working without taking the proper time to create an abstraction. However, as some very experience developers I watch also say that abstraction is not free and comes with a cost, it has diminishing returns, abstract too early, and you're in for a nightmare. 

As I worked on the project I kept that in mind. I worked in manageable sizes for tasks. I got the bottle drawn and then encapsulated it into it's own function. I did the same with the table as well. I also refactored the code so the RenderScene method would stay cleaner, by utilizing void function pointers I was able to create for the most part a general function to draw meshes, but it had limitations and would'nt draw every mesh.
